...
class AdministrativeEvent(models.Model):
	_name = "agendaesi.administrativeevent"
	...
	calendar_id = fields.Many2one("agendaesi.administrativecalendar", ondelete="cascade", string="Calendar", required=True)
	...
	def _compute_can_subscribe(self):
		for record in self:
			can_subscribe = self.env.user.has_group('agendaesi.agendaesi_administrative') or self.env.user.has_group('agendaesi.agendaesi_root')
	...
	def create(self, values):
		if values['periodicity'] is not None and values['end_periodicity'] is not None:
			...
			while curdate < fields.Datetime.from_string(values['end_periodicity']):
				...
				super(AdministrativeEvent, self).create(cp_values)
			
			
		ev = super(AdministrativeEvent, self).create(values)
		return ev
