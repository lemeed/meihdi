...
class PedagogicEvent(models.Model):
	_name = "agendaesi.pedagogicevent"
	...
	calendar_id = fields.Many2one("agendaesi.pedagogiccalendar", ondelete="cascade", string="Calendar", required=True)
	...
	def _compute_can_subscribe(self):
		for record in self:
			can_subscribe = self.env.user.has_group('agendaesi.agendaesi_teacher') or self.env.user.has_group('agendaesi.agendaesi_root')
	...
	def create(self, values):
		if values['periodicity'] is not None and values['end_periodicity'] is not None:
			days_treshold=1
			...
			while curdate < fields.Datetime.from_string(values['end_periodicity']):
				...
				super(PedagogicEvent, self).create(cp_values)
			
			
		ev = super(PedagogicEvent, self).create(values)
		return ev
